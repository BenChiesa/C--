...
reverse order that inversion count is maximum. Formally speaking, two elements a[i]
and a[j] form an inversion if a[i] > a[j] and i < j
Example: The sequence 2, 4, 1, 3, 5 has three inversions (2, 1), (4, 1), (4, 3).
What would the complexity of a brute force approach be? Code an O(nlog(n))
algorithm to count the number of inversions. Hint - piggy back on the merging step
of the mergesort algorithm.
The following two videos from Prof. Roughgarden may help you understand the problem
better. Watch the first video and then try to figure out a good algorithm before
